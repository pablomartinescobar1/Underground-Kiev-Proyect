...
data_file_name = "data/datos.csv"
image_file_name = "data/metro_Kiev_1155x796.png"
...
def crear_boton(label, n): #Crea un boton con texto n que al ser pulsado cambia la etiqueta label por n
    return tk.Button(ventana, text = n,command = lambda: cambiar_label(label, n))
...
def colocar_widgets(l1,l2,l3,decision): # Coloca los botones en la ventana exactamente en las posiciones (X, Y) de los pixeles que coinciden con su parada en la imagen de fondo
    ...
    for i in range(3):
        if (decision[0][i]==1):
            l1[i].place(x=160, y=295 + i * 41)
    for i in range(6):
        if (decision[0][i+3]==1):
            l1[i + 3].place(x=160 + i * 52, y=430)
    if (decision[0][9] == 1):
        l1[9].place(x=463, y=390)
    for i in range(3):
        if (decision[0][i+10]==1):
            l1[i + 10].place(x=540 + i * 67, y=370)
    for i in range(4):
        if (decision[0][i+13]==1):
            l1[i + 13].place(x=735 + i * 45, y=350 - i * 35)
    if (decision[0][17] == 1):
        l1[17].place(x=905, y=215)

    #l2:
    for i in range(8):
        if (decision[1][i]==1):
            l2[i].place(x=540, y=40 + i * 43)
    for i in range(5):
        if (decision[1][i+8]==1):
            l2[i + 8].place(x=540, y=450 + i * 60)
    for i in range(2):
        if (decision[1][i+13]==1):
            l2[i + 13].place(x=490 - i * 80, y=733)
    for i in range(3):
        if (decision[1][i+15]==1):
            l2[i + 15].place(x=340 - i * 60, y=733)
    #l3:
    for i in range(4):
        if (decision[2][i]==1):
            l3[i].place(x=325 + i * 40, y=170 + i * 62)
    if (decision[2][4] == 1):
        l3[4].place(x=570, y=445)
    for i in range(7):
        if (decision[2][i+5]==1):
            l3[i + 5].place(x=600 + i * 26, y=465 + i * 36)
    if (decision[2][12] == 1):
        l3[12].place(x=780, y=710)
    for i in range(3):
        if (decision[2][i+13]==1):
            l3[i+13].place(x=815 + i * 37, y=735)
...
def boton_calcular_ruta():
    ...
    borrar_widgets()
    reiniciar.place(x=780, y=80)
    tiempo_text.place(x=800, y=450)
    tiempo_label.place(x=940, y=450)
    fecha_text.place(x=800, y=500)
    fecha_label.place(x=910, y=500)
...
def colocar_widgets_entrada():
    origen_text.place(x=700, y=60)
    origen_entry.place(x=750, y=60)
    fijar_origen.place(x=780, y=60)
    seleccionar_origen.place(x=700, y=82)
    destino_text.place(x=700, y=150)
    destino_entry.place(x=750, y=150)
    fijar_destino.place(x=780, y=150)
    seleccionar_destino.place(x=700, y=172)
    ...
    origen_text2.place(x=900, y=450)
    origen_label.place(x=950, y=450)
    destino_text2.place(x=900, y=475)
    destino_label.place(x=950, y=475)
    boton_algoritmo.place(x=900, y=500)
...
ventana = tk.Tk()
ventana.geometry("1155x796") # Mismo tamaño que el png
...
circulos_l1 = []
for i in range(18):
    circulo = tk.Canvas(ventana, width=25, height=25, bg="red")
    circulo.create_oval(2, 2, 25, 25, fill="yellow")
    circulo.create_text(13, 13, text=i+110)
    circulos_l1.append(circulo)
circulos_l2 = []
for i in range(18):
    circulo = tk.Canvas(ventana, width=25, height=25, bg="blue")
    circulo.create_oval(2, 2, 25, 25, fill="yellow")
    circulo.create_text(13, 13, text=i+210)
    circulos_l2.append(circulo)
circulos_l3 = []
for i in paradas_l3:
    circulo = tk.Canvas(ventana, width=25, height=25, bg="green")
    circulo.create_oval(2, 2, 25, 25, fill="yellow")
    circulo.create_text(13, 13, text=i)
